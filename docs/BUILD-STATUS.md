...
| **DEF-STMT-INGEST** | Populate `public.financial_statements` (≥8Q) so the (built) ratio + Score path has real inputs. **Persist HALF: ✅ DONE 2026-07-16** (source-agnostic). **Producer HALF: OPEN** — wire a source that emits `NormalizedStatementRow` into the persist contract. | **✅ Persist layer shipped (migration `20260716095100`, source-agnostic):** `FILING.FINANCIALS` lake.object → `public.financial_statements` projection (`lake.fn_financial_statement_project`, mirrors `fn_filing_project` 0037) + **restatement versioning** (`version`/`is_restated`/`restated_at` on the current row + an append-only `public.financial_statement_history` — a restated quarter ARCHIVES the prior version then overwrites in place with a bumped version, so readers keep exactly one current row per period and the trailing-quarter TTM roll-up can never double-count; validated on PG17: first-file→v1, identical re-project→no-op, restatement→v2+archived, RETIRED→no re-write, unknown-ticker→skip). Plus the `NormalizedPeriod→FILING.FINANCIALS` staging mapper (`runtime.mapStatement` + `flattenStatements`) and `financials` data_type routing (`VenueAdapter.financials`, `tasksForDataType` case, `DATA_TYPE_TO_HANDLER`→`quote_poll`). **INERT until a producer is seeded** (no `financials` `ingest.sources` row on main → trigger never fires → zero live-pipeline change). **Owner steer 2026-07-16: AVOID Mubasher** (aggregator = paywall/durability risk for the Financials tab + Score) — so the Mubasher `/financial-statements`+`/ratios` adapter was **NOT** built; the golden fixtures (`fixtures/mubasher/financial-statements-2222.json`, `ratios-2222.json`) + `normalizeMubasherStatements` stay as the tested normalizer path any future aggregator could reuse. | **Producer HALF pickup:** wire a **free** statement source that emits `NormalizedStatementRow[]` via a `financials` `TaskSpec` + `ingest.sources` seed → statements auto-flow staging→cross_check→projection. **Preferred producer = the deterministic Tadawul XBRL feed** ("$0, proven live") currently stranded on the off-main branch `claude/focused-khayyam-ef4936` (51 commits behind main, its `20260713000042_financials_project.sql` **collides** with main's `0042_mubasher_deep_ohlcv`). That branch must be **rebased onto main + its 0042 collision resolved + its projection reconciled against this one** (same `FILING.FINANCIALS` object_type + serving table) before it can feed this contract. Then verify statements land + the ratios cron produces non-null `pe` + `score_batch` computes. Per-security producers must add a completion-stamp column + extend the `runTask` chunk/coverage-skip (both currently scoped to `ohlcv_backfill`). | `07 §P1.7b`, `plan §3a` |
