...
| **DEF-WORKER-DEPLOY-SECRETS** | Activate the CI worker deploy: set repo secrets `VPS_HOST` (= `91.99.99.85`) + `VPS_SSH_KEY` (deploy private key). **These two are all that's needed** — the workflow now defaults `VPS_USER=deploy` and `APP_DIR=/opt/marsad` (both per cloud-init), uses `sudo` for the restart (deploy is non-root), triggers on `ingestion/**`, and builds `ingestion/dist` before the worker (runs compiled `node dist/index.js`). Until the secrets are set `worker-deploy` **skips** every run (preflight `configured=false`) → the VPS worker only updates by manual SSH, so merged `worker/**` and `ingestion/**` changes do NOT reach prod. | **Owner sets the two secrets** (one-time), then `gh workflow run worker-deploy.yml --ref main`. Manual deploy meanwhile: `cd /opt/marsad && git fetch origin && git reset --hard origin/main && (cd ingestion && npm ci && npm run build) && (cd worker && npm ci && npm run build && npm prune --omit=dev) && sudo systemctl restart marsad-worker` | `worker-deploy.yml`, `infra/cloud-init.yaml` |
