...
| **DEF-RESEARCHER-GUARDRAILS** | The two Tadawul browser researchers (`marsad-researcher` XBRL + `marsad-gapfill` fsPdf, VPS `/home/deploy`) burned **~9 GB/night** on the metered proxy — full-page headed-Chromium SPA loads through Geonode, every 15/20 min, no resource interception, re-loading pages even when nothing is new (root cause `08-worker-fleet.md §5`). **Mitigated 2026-07-16 (reversible):** both timers throttled `15/20 min → 6 h` (systemd drop-ins) → ~20× fewer runs, ~9 GB → a few hundred MB. **Deeper fixes: (2)(3)(4) ✅ DONE 2026-07-16; (1) specced, needs owner OK.** (2) resource interception (`context.route` block image/font/media/trackers) + bytes/run logging — shipped as `scripts/researchers/scrape-guardrails.mjs`, both scrapers import it, validated live (Aramco: 17 XBRL/21 PDF found, 137 reqs blocked, no extra Akamai challenge, `proxy 10.8MB` logged); (4) hard per-run byte budget (`MAX_RUN_BYTES`, default 800MB) — shipped in the same helper; (3) both scrapers + wrappers + systemd units now repo-tracked under `scripts/researchers/`. **(1) event-driven cadence** (re-scrape a company only on a new financial filing, not a blind timer) — specced in `08 §6`: Option A calendar-window gate (near-term, ~15 lines) + Option B cheap direct Mubasher/Argaam disclosures detector → targeted scrape (endgame). | (2)(3)(4) shipped; the throttle + interception already take ~9GB to <1GB/night. | **(1):** owner sign-off on Option A (implement now) + the Mubasher/Argaam disclosures discovery task (Option B prerequisite). Also: repoint the systemd units at `/opt/marsad/scripts/researchers/` so the VPS runs the git checkout (kills the manual `/home/deploy` sync). | `08-worker-fleet.md §5/§6`, `scripts/researchers/` |
