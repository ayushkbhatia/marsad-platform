...
| **DEF-VENUE-FILINGS-BHB-PDF** | BHB per-announcement **PDF** download is deferred — BHB filings are LIST-ONLY. The AnnouncementDetail page (the list ref's `linkUrl`) does NOT serve the real attachment in static HTML: it loads client-side via SharePoint `_vti_bin/spsdisco.aspx`, and the only `.pdf` hrefs served are site **chrome** (footer Code-of-Ethics / Corporate-Governance docs), so an HTML scrape grabs the same wrong PDF for every announcement (verified + cleaned up 2026-07-16). The BHB `filing_detail` source (id37) is deactivated (`20260716094500`); `FILING_PDF_RESOLVERS` is unwired for BHB. | The static-HTML path yields only chrome PDFs; the real doc needs the SharePoint attachment/webapi endpoint. | Capture BHB's attachment endpoint (the `spsdisco.aspx` doc-URL call, or a `GetAnnouncementDetail?itemid=` webapi returning the file URL — same host + dynamic Bearer as the list), then set `pdf_url` on the ref (or add a BHB resolver) and reactivate id37. | `adapters/bhb/filings.ts`, `adapters/filings-detail.ts` |
| **DEF-VENUE-FILINGS-DFM-PDF** | DFM filings are LIST-ONLY (detail id33 deactivated, `20260716095000`). The eFsah feed's `resources[].r_path` (e.g. `/2026/Jul/15/<uuid>/BOD Meeting.Pdf.pdf`), which `adapters/dfm/filings.ts` joins to `api2.dfm.ae`, returns **HTTP 404 `{"statusCode":404,"message":"Resource not found"}`** for a plain GET (verified from the VPS across host/prefix variants). So `api2.dfm.ae` is the list-feed host, not the resource-download host. | Downloading the wrong (404) URL burns ~20 futile fetches per DFM filing-burst; list still publishes, so nothing downstream starves. | Capture the REAL eFsah PDF-download host: navigate the DFM disclosures page headless, open a filing, capture the actual attachment URL (`page.on('response')` for the `.pdf` request), then fix `resolvePdfUrl` in `adapters/dfm/filings.ts` + reactivate id33. | `adapters/dfm/filings.ts` |
