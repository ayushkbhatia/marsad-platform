...
| **BHB** (Bahrain) | ✅ **webapi board** `GetTabularData?storedProcdure=Quotes` (sticky proxy, `20260715100000`) | ⚠️ filings URL 403 (DEF-VENUE-FILINGS) | datacenter IP blocked + Cloudflare → **sticky** IPRoyal; live board = 41 stocks incl. daily OHLCV. Quotes fix pending worker deploy |
...
| **DEF-VENUE-FILINGS** | Venue filings parser tuning — QE (q-disclosure URL), BHB (webapi/proxy), DFM (eFsah drift), MSX (RSS drift), TDWL (pin Mubasher-news). **+ BHB quotes (id16).** | Low-volume + parsers **drift** (ongoing maintenance, never a finish line); does not gate the Score. **Root-caused 2026-07-15:** id2/id11/id17 fail because `urlTemplate` was never pinned → `browser.bootstrap()` throws `action discovery found no URL` (`core/browser.ts:166-170`), 0/287 ok, parser never runs; id16 BHB quotes falls back to the SPA-shell HTML → JSON parser (`adapters/bhb/quotes.ts:90`) gets HTML → 0 rows. Not selector drift. | **P1.7d** filings upgrade / when reader-needed. **Turnkey fix per venue:** capture the real feed XHR once via VPS headless (`page.on('response')`) → pin `endpoint_config.urlTemplate` (+ `filingFieldMap` if non-standard) → mirror `sources.seed.ts` → add golden fixture → re-`active=true`. TDWL `saudiexchange.sa` issuer-news; QE `qe.com.qa/q-disclosure` portlet XHR; BHB quotes `webapi.bahrainbourse.com/api` (also verify proxy 402); BHB filings urlTemplate already pinned — drop `actionDiscovery` so `fetchFilings` uses it directly (golden `fixtures/bhb/filings-live.json` exists). | `P1.7d`, `01-ingestion.md` |
| **DEF-EOD-BULLETIN** (added 2026-07-15) | ✅ **ROOT-CAUSED + FIXED 2026-07-15** — no `eod_bulletin` source ever produced a single `ingest.fetch_log` row (incl. ADX id9 / BHB id18), despite hourly `eod_sweep` jobs finishing `status='ok'`. **NOT missing calendar data**: `public.market_sessions` (6 venue rows, close times correct) and `public.venues.timezone` are all populated. Root cause: `eodCloseGate` (`worker/src/handlers/eod-sweep.ts`) selected `(ts)::date` / bare-timestamp columns; postgres.js parses `date`/`timestamp` into JS `Date` objects (`db.ts` registers no custom parsers), so `row.local_date !== tradeDate` (a `'YYYY-MM-DD'` string from the poller's `to_char`) was **always true** → gate permanently closed → handler skipped cleanly every tick (job `'ok'`, zero fetches, zero errors). Second latent bug fixed in the same pass: `local_ts.slice(11,19)` on a `Date` would have thrown once the date check passed. Fix: gate SQL now returns text end-to-end (`to_char` for `local_time`/`local_date`, `close_local::text`); regression test pins the casts. **Proven against the live DB**: the fixed CTE evaluates OPEN at close+30 for ADX (15:30 GST) and BHB (13:30 AST), `before_close`/`past_post_close_window` controls correct. Remaining: first live-window validation. | Fix landed same day; only the live confirmation is outstanding (today's windows had already passed at deploy time). | **Validate 2026-07-16 post-close**: after ADX 15:00 GST / BHB 13:00 AST (+180-min window), confirm `ingest.fetch_log` rows for sources 9/18 and staged `OHLCV.CLOSE`, then strike this row. TDWL/DFM/QE/MSX eod_bulletin remain deliberately deactivated (`20260715150100`) — do not re-enable. | `worker/src/handlers/eod-sweep.ts`, `07 §P1.7a` |
