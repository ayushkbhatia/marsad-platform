...
| **DEF-RESEARCHER-GUARDRAILS** | The two Tadawul browser researchers (`marsad-researcher` XBRL + `marsad-gapfill` fsPdf, VPS `/home/deploy`) burned **~9 GB/night** on the metered proxy — full-page headed-Chromium SPA loads through Geonode, every 15/20 min, no resource interception, re-loading pages even when nothing is new (root cause `08-worker-fleet.md §5`). **Mitigated 2026-07-16 (reversible):** both timers throttled `15/20 min → 6 h` (systemd drop-ins) → ~20× fewer runs, ~9 GB → a few hundred MB. **Deeper fixes deferred (touch the proprietary scraper — need owner OK):** (1) event-driven cadence (re-scrape a company only on a new financial filing, not a blind timer); (2) resource interception (`context.route` block image/font/media/trackers) + bytes/run logging, validating Akamai challenge rate; (3) move the scrapers into the repo (`scripts/researchers/`), version-controlled + config-declared; (4) a hard per-run byte budget. | Throttle solved the acute cost; the deeper fixes are efficiency + hygiene, and the event-driven rewrite is an owner decision on their proprietary scraper. | **Now** for the repo-tracking + byte logging (low-risk); **owner sign-off** for the event-driven rewrite. Or if proxy cost is still material after the throttle. | `08-worker-fleet.md §5`, VPS `/home/deploy/tadawul-{researcher,gapfill}.mjs` |
