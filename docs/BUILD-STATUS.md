...
| **DEF-WORKER-DEPLOY-SECRETS** | Activate the CI worker deploy: set repo secrets `VPS_HOST` + `VPS_SSH_KEY` (owner-only). Until then `worker-deploy` **skips** every run (preflight `configured=false`) → the VPS worker only updates by manual SSH, so merged `worker/**` **and** `ingestion/**` changes do NOT reach prod automatically. **Also verify** `vars.VPS_APP_DIR`: the service runs from `/opt/marsad/worker` (WorkingDirectory) but the deploy defaults `APP_DIR=/opt/marsad-platform` — must point at the repo root whose `worker/` is the unit's WorkingDirectory. Workflow itself is now correct (triggers on `ingestion/**`, builds `ingestion/dist` before the worker — the worker runs compiled `node dist/index.js`). | **Owner sets the two secrets** (one-time). Manual deploy meanwhile: `cd <repo> && git reset --hard origin/main && (cd ingestion && npm ci && npm run build) && (cd worker && npm ci && npm run build && npm prune --omit=dev) && systemctl restart marsad-worker` | `worker-deploy.yml`, `infra/marsad-worker.service` |
