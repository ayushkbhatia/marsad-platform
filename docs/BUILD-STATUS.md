...
|---|---|---|---|---|
| **DEF-STORAGE-PURGE-WORKER** | Worker consumer for `ops.storage_purge_queue`: delete freed `lake-raw` objects via the Storage API, stamp `deleted_at`. Context: the 0013 retention purge was a **silent no-op** (matched dot-delimited `'%.quotes%'` keys, but `source_key` is colon-delimited `TDWL:quotes:1`; JSON boards missed the `text/html` fallback) — fixed 2026-07-15 in `20260715185413` (pattern `'%:quotes:%'`, JSON 1-y branch, financials excluded). The purge also only **nulls** `storage_path`; Postgres cannot delete Storage backing files, so without a consumer every purged blob (1,188 quote objects and counting) leaks in `lake-raw` forever once purges start firing. | Purge fires first ~2026-10-11 (oldest snapshot 2026-07-13 + 90 d) — no orphans exist until then, so the queue safely accumulates. Small handler: pgmq-less poll of `storage_purge_queue where deleted_at is null` + Storage remove + stamp. | **Before 2026-10-11** (first real purge), or with the next worker-handler batch — whichever first | `ops.storage_purge_queue` (`20260715185413`), `02 §3/§20`, worker `handlers/` |
