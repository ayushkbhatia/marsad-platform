...
| **BHB** (Bahrain) | ✅ **webapi board** `GetTabularData?storedProcdure=Quotes` — **LIVE, direct egress** (`20260715100000` + `20260715150100`) | ✅ **filings live** (`20260716093000`) — `GetAllAnnouncements` webapi + dynamic Bearer, 1005 announcements; filing_detail chain → PDF bucket + extraction queue | webapi WAF gate is gone → **direct** VPS egress (proxy off, bearer rotated, replay-fatal `provider` key stripped — `20260715150100`); live board = 41 stocks incl. daily OHLCV |
...
| **DEF-VENUE-FILINGS-BHB** | ✅ **DONE 2026-07-16** — BHB filings_list (id17) reactivated (`20260716093000`). Root cause was the `actionDiscovery` network_capture (the SharePoint page never fires the XHR → `browser.bootstrap()` threw); dropped it + moved to plain `http` (`20260716091000`). `adapters/bhb/filings.ts` now fetches the pinned `GetAllAnnouncements` webapi with the **shared dynamic APIKey Bearer** (`bhb/webapi.ts`, extracted from quotes so both share one rotating token) — confirmed live from the VPS: GET 200, `status:1`, 1005 announcements, `itemid` stable per-filing id. | — | — | `adapters/bhb/filings.ts`, `adapters/bhb/webapi.ts` |
