...
| **DEF-STMT-LLM-PDF** | Implement `normalizeViaLlm` (the declared seam in `statement-normalizer.ts`) + the PDF-extraction pipeline for MSX/BHB statement depth + F full-text. | **Largely built** (2026-07-14): the extraction contract + validation gate (`statement-extraction.ts`, tested), migration 0042 `fn_financials_project`, and the Argaam discovery parser (`adapters/argaam/discover.ts`) are committed + a live projection proof done. Remaining = the impure orchestrator (pdftotext+LLM) + Argaam headless fetch/acquire/store. | Gated on **DEF-WORKER-LLM-KEY** below | `p17-financials-pdf-architecture.md` |
| **DEF-WORKER-LLM-KEY** ⚠️ **owner action** | Add an LLM provider key (`ANTHROPIC_API_KEY` or `OPENROUTER_API_KEY`) to `/etc/marsad/worker.env` on the VPS + restart. The worker has **never** called the LLM gateway (P3-era); `worker.env` today has only Supabase creds. | **Blocks automated financials extraction** (and all future worker LLM: filing summaries, AI profiles). The extraction code (`statement-extraction.ts`) + the projection are built + proven; they can't run auto on the VPS until a key exists. Also needs the worker→`src/lib/llm` build wiring (the worker is a separate package with no path to the gateway). | Before the Argaam financials backfill can auto-extract | `p17-financials-pdf-architecture.md §6 D-fin-2`, `06-infra-cost.md §9` |
| **DEF-SHARES-OUTSTANDING** | Source `securities.shares_outstanding` (all 660 null) — gates `market_cap` → P/B, P/S, EV/EBITDA (the P/B-family of the Value factor). Yahoo `defaultKeyStatistics` has it but Yahoo is egress-blocked. | E/P + dividend-yield still compute Value; Growth/Profitability/Momentum unaffected — so a 3-factor Score still ships. But P/B (the primary bank gauge, D-1) needs shares. | With the proxy (D-src-3) or a shares source | `07 §3.2`, `p17-continuous` |
