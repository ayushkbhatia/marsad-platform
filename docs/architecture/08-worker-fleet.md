...
| **stockanalysis-financials** (transient `systemd-run` unit `marsad-sa-fin-<venue>`, per venue — **no repo `.timer`, run ad-hoc**) | Second-source financials cross-check (`FINANCIALS.XCHECK`, source_rank 90): per security, stockanalysis.com `__data.json` (S&P-Global re-served) → land PENDING `lake.objects` → the reconcile trigger writes agree/conflict/gap into `financial_statement_xcheck`. **Never touches golden `financial_statements`.** | **direct HTTP — NO browser, NO proxy, NO LLM.** curl/fetch the SvelteKit loader JSON per ticker | ad-hoc (owner-run `systemd-run` per venue: `VENUE=TDWL\|QE\|ADX\|MSX\|BHB\|DFM`); no timer, not in `scripts/researchers/systemd/` | run-deadline via the loop; incremental (skips `sa_symbol` misses); closes its `lake.parse_runs` `failed` on crash (fixed 2026-07-26) | env: `VENUE/REFRESH/MAX`; header comment documents the `systemd-run` invocation |
| **msx-financials** (`marsad-msx-financials.timer` → `msx-financials-cron.sh` → `msx-financials.mjs`) — **the cheapest researcher in the fleet** | MSX financial reports: `snapshot.aspx/FinancialsReports` index (one POST = a company's ENTIRE history) → download the report's English **ZIP** → unzip → archive every member PDF to the `filings` bucket + catalogue ONE `public.filings` row per report (manifest in `extracted_facts`). **Filings only — no extraction** (deferred, DEF-MSX-STMT-EXTRACT) | **direct HTTP — NO browser, NO proxy, NO LLM, NO OCR.** MSX serves index + artifacts COLD (no cookie/session/ViewState; Incapsula fronts the site but does not gate these calls). Members are pre-segmented per statement and are digital text rendered from XBRL — so no `pdftotext`/`claude` step is needed to ARCHIVE them. **Host dep: `unzip`** (`infra/cloud-init.yaml`); `preflight()` exits 1 if absent | **6 h** (no window-gate — see note) | `ZIP_MAX` = **download ATTEMPTS**/run (spent on the attempt, not the success — else a deterministic failure re-downloads unthrottled forever) + `RUN_BUDGET_MS` + `FETCH_TIMEOUT_MS` (node `fetch` has NO default timeout) + `MAX_ZIP_BYTES` (compressed) / `MAX_UNZIPPED_BYTES` + `MAX_MEMBERS` (**checked via `unzip -l` BEFORE extracting** — DEFLATE hits ~1000:1, so extract-then-cap is no guard at all); **incremental skip-owned BEFORE download** (`public.filings` `MSX-FS-*`); **all-or-nothing upload** (a partial member set is never catalogued/owned); permanent-vs-transient failure split (permanent ⇒ owned marker + `extracted_facts.msx_report.archive_error`, never retried); own `.msx-financials.lock` | env: `CHUNK_SIZE/ZIP_MAX/MSX_YEARS_BACK/RUN_BUDGET_MS/FETCH_TIMEOUT_MS/MAX_ZIP_BYTES/MAX_UNZIPPED_BYTES/MAX_MEMBERS`; state `.msx-financials-chunk` |

> **`bhb-financials`, `adx-gapfill` and `msx-financials` are the CHEAP script-driven researchers:** all are
> script-driven like the Tadawul pair, but **NOT a proxy/bandwidth risk** — the "most expensive thing we
...
>
> **Shared `lake.objects` write path (all researchers):** every researcher that lands objects now writes
> through `scripts/researchers/lib/lake-objects.mjs` (`upsertLakeObject`) — ONE `sql.begin()` transaction
> per natural key (so the `deferrable` `superseded_by` self-FK resolves at COMMIT) + `on conflict
> (natural_key, revision) do nothing` with a single retry. Do NOT hand-roll a `select … superseded_by is
> null` → `insert … revision 1` sequence again: as separate autocommits it races two overlapping runs into
> a `one_live_per_key` crash (killed `marsad-saf-TDWL` 2026-07-21) and its VERIFIED-supersede branch cannot
> work under autocommit. See DEF-LAKE-OBJECTS-RACE in `BUILD-STATUS.md §7`.
>
> **`msx-financials` is cheaper still — it is the only statement researcher with NO LLM budget at all,**
> because it archives filings rather than extracting them (DEF-MSX-STMT-EXTRACT). Verified live 2026-07-17:
> `snapshot.aspx/FinancialsReports` and `/MSMDOCS/FinancialReports/*.zip` both answer **cold from plain
> `curl`** — no cookie, no session, no ViewState, no WAF challenge. Its steady-state cost is ~108 index
> POSTs ≈ **5 MB/run (~21 MB/day)** and zero downloads once backfilled, because the owned-skip happens
> BEFORE any artifact fetch.
>
> **Why `msx-financials` has NO window-gate (deliberate, do not "fix" this):** `window-gate.sh` grants its
> long `ANNUAL_WINDOW_DAYS` only to a **Dec-31** year-end — a *Saudi* assumption. MSX fiscal years vary by
> issuer (AAIC's FY ends **31 Mar**). Measured over the last 5 years, MSX issuers file quarterlies up to
> **47 d** after period-end and annuals up to **74 d**, with the Mar-31 annual filers landing at **63–72 d**
> — i.e. a Mar-31 *annual* would fall outside the gate's 45 d *quarterly* window and be skipped. A window
> wide enough to catch them (~85 d on every quarter-end) spans ~340/365 days and would never throttle
> anything. So the gate buys nothing here and only risks dropping stragglers; the unconditional 6 h cadence
> is both cheaper to reason about and fresher.
