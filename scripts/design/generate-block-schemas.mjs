 ...
 *
 * `src/lib/blocks/schemas/` is the single source of truth; the DB column is a projection of it, the
...
import '../../src/lib/blocks/schemas/ts-resolve.mjs';
...
} = await import(join(ROOT, 'src/lib/blocks/schemas/index') + '.ts');
...
const SQL = [
  ...
  '-- Source of truth: src/lib/blocks/schemas/*.ts (Zod v4). This column is the PROJECTION —',
  ...
  '  --     Same predicate as carriesObjectBinding() in src/lib/blocks/schemas/index.ts: an object',
...
if (CHECK) {
  if (!existsSync(OUT) || readFileSync(OUT, 'utf8') !== sql) {
    console.error('✗ payload_schema migration STALE vs src/lib/blocks/schemas — regenerate and review the diff');
    process.exit(1);
  }
