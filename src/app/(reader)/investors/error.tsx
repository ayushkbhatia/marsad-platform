...
/** Segment error boundary (App Router error.tsx must be a client component). Covers the
 *  directory and `[slug]` detail — both live under this segment. */
export default function InvestorsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("[investors]", error);
  }, [error]);
    ...
    <div className="mx-auto max-w-[1180px] px-5 py-16 sm:px-8">
      <div className="flex flex-col items-center gap-3 border border-dashed border-hairline bg-paper-tint/60 px-6 py-14 text-center">
        ...
        <p className="max-w-[420px] font-display text-[17px] leading-tight text-ink-mid">
          Investors didn&apos;t load.
        </p>
        <p className="max-w-[440px] font-ui text-[12.5px] leading-relaxed text-ink-faint">
          Try again — if this keeps happening, the holders feed may be degraded.
        </p>
        <button
          type="button"
          onClick={() => reset()}
