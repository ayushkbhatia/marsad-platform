...
|---|---|---|---|---|---|
| `qe/marketwatch.txt` | QE | `https://www.qe.com.qa/pps/qse_files/MarketWatch.txt` | http | 200 | ⚠ **RETIRED URL** — this path is a *stale static file* (Last-Modified Oct 2025, byte-identical intraday). Live board is `POST /wp/mw_app/mw.php` body `f=MarketWatch` (migration `20260715085252`). Same schema, so the quotes golden still tests the real parse path, but 132 KB/107 rows vs a live 461 KB/126 rows. See `qe/README.md`. |
...
| `qe/fs-*.json` | QE | `qdisclosure/api/XBRL/GetFinancialStatementsAPIData?…` | http | 200 | **2026-07-17.** XBRL facts as JSON — the goldens for `adapters/qe/financials.ts`. Cover all three taxonomies (`ifrs-full_`, Islamic-bank `qse-crc_`, Takaful `qse-ins-is_`). Stored **sorted**: the API has no ORDER BY and returns a different row order every call. |
| `qe/filing-details-*.json` | QE | `…GetFinancialStatementsAPIData?…&getFilingDetails=1` | http | 200 | `{approvedDate, uploadedDate}` → `filedAt`. |
| `qe/README.md` | QE | — | — | — | Capture commands, the retired-URL note, the three-taxonomy identity landmine, coverage/depth facts. |
